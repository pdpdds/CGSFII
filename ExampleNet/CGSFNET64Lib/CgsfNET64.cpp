...
namespace CgsfNET64Lib {
	...
	bool CgsfNET64::AddTimer(int timerID, DWORD period, DWORD delay)
	{
		return SFEngine::GetInstance()->AddTimer(timerID, period, delay);
	}
	...
	bool CgsfNET64::AddPacketProtocol(int protocolID, int maxBufferSize, int maxPacketSize, int option)
	{
		...
		if (SFEngine::GetInstance()->AddPacketProtocol(packetProtocol) < 0)
		{
			return false;
		}
