...
/***********************************************************
  ...
  Latest Changes:
		...
		02/06/2002	  Fixed a bug that would cause an application to hang.
						...
						when it called CDirectoryChangeWatcher::UnwatchDirectory(const CStdString & )
		...
		02/09/2002	Added a parameter to CDirectoryChangeHandler::On_ReadDirectoryChangeError()
					...
					Added the parameter: const CStdString & strDirectoryName
					The new signature is now:
							virtual void CDirectoryChangeHandler::On_ReadDirectoryChangeError(DWORD dwError, const CStdString & strDirectoryName);
		...
		04/25/2002	Added two new virtual functions to CDirectoryChangeHandler
					...
					Added:
							On_WatchStarted(DWORD dwError, const CStdString & strDirectoryName)
							On_WatchStopped(const CStdString & strDirectoryName);
		...
		04/27/2002  Added new Parameters to CDirectoryChangeWatcher::WatchDirectory()
					...
					Signature is now:
					CDirectoryChangeWatcher::WatchDirectory(const CStdString & strDirToWatch, 
...
static bool IsDirectory(const CStdString & strPath);
/////////////////////////////////////////////////////////////////////
//	Helper functions.
...
public:
	...
	CStdString GetFileName()const;//gets the file name from the FILE_NOTIFY_INFORMATION record
	CStdString GetFileNameWithPath(const CStdString & strRootPath) const;//same as GetFileName() only it prefixes the strRootPath into the file name
...
CStdString CFileNotifyInformation::GetFileName() const
{
	//
	//BUG FIX:
	...
	if( m_pCurrentRecord )
	{
		...
		memcpy(	wcFileName, 
				...
				//min( MAX_PATH, m_pCurrentRecord->FileNameLength) <-- buggy line
				 ((MAX_PATH * sizeof(WCHAR)) < m_pCurrentRecord->FileNameLength) ? (MAX_PATH * sizeof(WCHAR)) : m_pCurrentRecord->FileNameLength);
		

		return CStdString( wcFileName );
	}
	return CStdString();
}		

static inline bool HasTrailingBackslash(const CStdString & str )
{
	if( str.GetLength() > 0 
...
CStdString CFileNotifyInformation::GetFileNameWithPath(const CStdString & strRootPath) const
{
	CStdString strFileName( strRootPath );
...
//
//
void CDirectoryChangeHandler::On_FileAdded(const CStdString & strFileName)
{ 
	TRACE(_T("The following file was added: %s\n"), strFileName);
}

void CDirectoryChangeHandler::On_FileRemoved(const CStdString & strFileName)
{
	TRACE(_T("The following file was removed: %s\n"), strFileName);
}

void CDirectoryChangeHandler::On_FileModified(const CStdString & strFileName)
{
	TRACE(_T("The following file was modified: %s\n"), strFileName);
}

void CDirectoryChangeHandler::On_FileNameChanged(const CStdString & strOldFileName, const CStdString & strNewFileName)
{
	TRACE(_T("The file %s was RENAMED to %s\n"), strOldFileName, strNewFileName);
}
void CDirectoryChangeHandler::On_ReadDirectoryChangesError(DWORD dwError, const CStdString & strDirectoryName)
{
	TRACE(_T("WARNING!!!!!\n") );
...
void CDirectoryChangeHandler::On_WatchStarted(DWORD dwError, const CStdString & strDirectoryName)
{	
	if( dwError == 0 )
...
void CDirectoryChangeHandler::On_WatchStopped(const CStdString & strDirectoryName)
{
...
void CDirectoryChangeHandler::SetChangedDirectoryName(const CStdString & strChangedDirName)
{
...
BOOL CDirectoryChangeWatcher::IsWatchingDirectory(const CStdString & strDirName)const
/*********************************************
...
**********************************************/
{
	SFLockHelper lock((SFLock*)(&m_csDirWatchInfo));
...
int	CDirectoryChangeWatcher::NumWatchedDirectories()const
{
	...
	int nCnt(0),max = m_DirectoriesToWatch.size();
	for(int i(0); i < max; ++i)
	{
...
DWORD CDirectoryChangeWatcher::WatchDirectory(const CStdString & strDirToWatch, 
									   DWORD dwChangesToWatchFor, 
									   CDirectoryChangeHandler * pChangeHandler,
									   BOOL bWatchSubDirs /*=FALSE*/,
									   LPCTSTR szIncludeFilter /*=NULL*/,
...
/*************************************************************
FUNCTION:	WatchDirectory(const CStdString & strDirToWatch,   --the name of the directory to watch
...
PARAMETERS:
		const CStdString & strDirToWatch -- specifies the path of the directory to watch.
...
**************************************************************/
{
	...
	{//completion port created/directory associated w/ it successfully
		...
		if( m_hThread == NULL )
		{
			...
			m_hThread = (HANDLE)_beginthreadex(0, NULL, MonitorDirectoryChanges, this, 0, (unsigned*)&m_dwThreadID);
			
			if( !m_hThread )
			{//couldn't create the thread!
				TRACE(_T("CDirectoryChangeWatcher::WatchDirectory()-- _beginthreadex failed!\n"));
				pDirInfo->DeleteSelf( NULL );
...
BOOL CDirectoryChangeWatcher::UnwatchAllDirectories()
{
	...
	if( m_hThread != NULL )
	{
		...
		for(int i = 0; i < max; ++i)
		{
			if( (pDirInfo = m_DirectoriesToWatch[i]) != NULL )
			{
				...
				m_DirectoriesToWatch.assign(i, NULL);
	...
	else
	{
		...
		{
			for(size_t i = 0; i < m_DirectoriesToWatch.size(); ++i)
			{
...
BOOL CDirectoryChangeWatcher::UnwatchDirectory(const CStdString & strDirToStopWatching)
/***************************************************************
FUNCTION:	UnwatchDirectory(const CStdString & strDirToStopWatching -- if this directory is being watched, the watch is stopped
...
****************************************************************/
{
	...
	if( m_hCompPort != NULL )//The io completion port must be open
	{
		...
		&&	nIdx != -1 )
		{
			...
			m_DirectoriesToWatch.assign(nIdx, NULL);
			pDirInfo->DeleteSelf(this);
			bRetVal = TRUE;
		}
	}
...
//	
{
	...
	if( pWatchInfo )
	{
		...
		if( GetDirWatchInfo(pWatchInfo, nIdx) == pWatchInfo )
		{
			...
			//
			m_DirectoriesToWatch.assign(nIdx, NULL);//mark the space as free for the next watch...
...
//
CDirectoryChangeWatcher::CDirWatchInfo * CDirectoryChangeWatcher::GetDirWatchInfo(const CStdString & strDirName, int & ref_nIdx)const
{
...
CDirectoryChangeWatcher::CDirWatchInfo * CDirectoryChangeWatcher::GetDirWatchInfo(CDirectoryChangeWatcher::CDirWatchInfo * pWatchInfo, int & ref_nIdx)const
{
	...
	int i(0), max = m_DirectoriesToWatch.size();
	CDirWatchInfo * p;
	for(; i < max; ++i)
	{
		if( (p = m_DirectoriesToWatch[i]) != NULL
...
CDirectoryChangeWatcher::CDirWatchInfo::CDirWatchInfo(HANDLE hDir, 
													  const CStdString & strDirectoryName, 
													  CDirectoryChangeHandler * pChangeHandler,
...
{ 
	//NOT SIGNALLED, MANUAL RESET
	m_StartStopEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
		...
		case FILE_ACTION_RENAMED_OLD_NAME:
			{//a file name has changed, and this is the OLD name
			 ...
			 //the action set to FILE_ACTION_RENAMED_NEW_NAME (contains the new name of the file
				...
				CStdString strOldFileName = notify_info.GetFileNameWithPath( pdi->m_strDirName );
				...
				{//there is another PFILE_NOTIFY_INFORMATION record following the one we're working on now...
				 //it will be the record for the FILE_ACTION_RENAMED_NEW_NAME record
					...
					CStdString strNewFileName = notify_info.GetFileNameWithPath( pdi->m_strDirName );
