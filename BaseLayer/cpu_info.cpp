...
char * CPUInfo::GetTypeID ()
{
	...
	static char szTypeID[32];
	_itoa_s (ChipID.Type, szTypeID, 10);
	return szTypeID;
}
...
char * CPUInfo::GetFamilyID ()
{
	...
	static char szFamilyID[32];
	_itoa_s (ChipID.Family, szFamilyID, 10);
	return szFamilyID;
}
...
char * CPUInfo::GetModelID ()
{
	...
	static char szModelID[32];
	_itoa_s (ChipID.Model, szModelID, 10);
	return szModelID;
}
...
char * CPUInfo::GetSteppingCode ()
{
	...
	static char szSteppingCode[32];	
	_itoa_s (ChipID.Revision, szSteppingCode, 10);
...
bool __cdecl CPUInfo::RetrieveProcessorSerialNumber ()
{
	...
	// Process the returned information.
	sprintf_s (ChipID.SerialNumber, "%.2x%.2x-%.2x%.2x-%.2x%.2x-%.2x%.2x-%.2x%.2x-%.2x%.2x",
	...
	switch (ChipManufacturer) {
		case Intel:
			...
			switch (ChipID.Family) {
				case 3:
					sprintf_s (ChipID.ProcessorName, "Newer i80386 family"); 
					break;
				case 4:
					switch (ChipID.Model) {
				...
				case 5:
					switch (ChipID.Model) {
						...
						case 8: STORE_CLASSICAL_NAME ("P55C (0.25)"); break;
				...
				case 6:
					switch (ChipID.Model) {
						...
						case 5: STORE_CLASSICAL_NAME ("Pentium?II (0.25)"); break;
						case 6: STORE_CLASSICAL_NAME ("Pentium?II With On-Die L2 Cache"); break;
						case 7: STORE_CLASSICAL_NAME ("Pentium?III (0.25)"); break;
						case 8: STORE_CLASSICAL_NAME ("Pentium?III (0.18) With 256 KB On-Die L2 Cache "); break;
						case 0xa: STORE_CLASSICAL_NAME ("Pentium?III (0.18) With 1 Or 2 MB On-Die L2 Cache "); break;
